...
PostT = 0.05; % post stim period % 0.25 for sxa
    ...
    for freq = 1:nFreq
        for condition = 1:nAmp
            ...
            elseif strcmp(stimuli_parameters.Par.Rec, 'SxA') % multimodal
                ...
                index = (strcmp(stimuli_parameters.Stm.MMType, 'SA')) & (stimuli_parameters.Stm.Amplitude == uAmp(condition)) & (stimuli_parameters.Stm.SomFreq == uFreq(freq));
                baseline = stimulusRate(control,cluster);
                stim_evoked = stimulusRate(index,cluster);
            end
...
for i = 1:length(conditions)
    ...
    dfiring(i, :) = abs(median(dstimulusRate(index, :)));
end

figure;
scatter(dfiring(2, :), dfiring(1,:)) % 1: SA, 2: SO
...
nClusters = length(cids);
responsive = responsive_units(3).responsive;
%nResponsive = length(responsive);
%resp_cells = [];
...
for cluster = 1:nClusters
    ...
    if max(ismember(responsive, cids(cluster)))
