...
% get directories
recordingFolder = ['D:\DATA\EphysRecordings\M' num2str(mousenr) '\' recSession '\'];
...
KSPath = fullfile('D:\DATA\EphysRecordingsSorted', ['M' num2str(mousenr) '\']);
BehaviorPath = fullfile('D:\DATA\Behavioral Stimuli', ['M' num2str(mousenr) '\']);
OutPath = fullfile('D:\DATA\Processed', ['M' num2str(mousenr)]);
...
for session = relevant_sessions(1):relevant_sessions(2)
    ...
    % load data
    [~, stimuli_parameters, ~, ~, ~, sessions_TTLs, ~, ~, ~] = loadData(OutPath, session, BehaviorPath);
